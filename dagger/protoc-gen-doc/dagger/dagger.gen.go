...
func (r ProtocGenDoc) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Ctr *Container
	}
	concrete.Ctr = r.Ctr
...
func (r *ProtocGenDoc) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		Ctr *Container
	}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "ProtocGenDoc":
		switch fnName {
		case "Generate":
			var parent ProtocGenDoc
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
			}
			var protoDir *Directory
			if inputArgs["protoDir"] != nil {
				err = json.Unmarshal([]byte(inputArgs["protoDir"]), &protoDir)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg protoDir", err))
				}
			}
			var docOpt string
			if inputArgs["docOpt"] != nil {
				err = json.Unmarshal([]byte(inputArgs["docOpt"]), &docOpt)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg docOpt", err))
				}
			}
			return (*ProtocGenDoc).Generate(&parent, protoDir, docOpt), nil
		case "":
			var parent ProtocGenDoc
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
	...
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("ProtocGenDoc").
					WithFunction(
						dag.Function("Generate",
							dag.TypeDef().WithObject("Directory")).
							WithDescription("Generate runs protoc-gen-doc on proto files, returning the generated documentation as a directory.\n\nExample usage: dagger call run --proto-dir /path/ --doc-opt \"markdown,docs.md\"").
							WithArg("protoDir", dag.TypeDef().WithObject("Directory"), FunctionWithArgOpts{Description: "The directory of the proto files."}).
							WithArg("docOpt", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{Description: "The doc_opt flag to pass to protoc-gen-doc.", DefaultValue: JSON("\"markdown,docs.md\"")})).
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("ProtocGenDoc")).
							WithArg("Image", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{Description: "ProtocGenDoc image to use.", DefaultValue: JSON("\"pseudomuto/protoc-gen-doc\"")}))), nil
	default:
