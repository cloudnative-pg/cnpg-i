...
func (r Protolint) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Ctr *Container
	}
	concrete.Ctr = r.Ctr
...
func (r *Protolint) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		Ctr *Container
	}
			...
			WithObject(
				dag.TypeDef().WithObject("Protolint").
					...
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("Protolint")).
							WithArg("Image", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{Description: "Protolint image to use.", DefaultValue: JSON("\"yoheimuta/protolint\"")}))), nil
	default:
